-- ФОРТОЧКА · схема базы данных (Supabase / Postgres)
...
-- создаём тип только если его ещё нет (безопасно при повторном запуске)
do $$ begin
...
create table if not exists articles (
  ...
  source_url    text unique not null,          -- дедупликация
  source_name   text not null,
  source_id     int references sources(id),
  raw_title     text not null,
  raw_text      text,
  raw_image_url text,                          -- картинка из RSS, если была
  ...
  tg_excerpt    text,
  reading_minutes int,
  image_url     text,                          -- финальная обложка (Storage)
  cover_svg     text,                          -- SVG-фолбэк, если нет фото
...
-- Этап 2: пользователи (Supabase Auth), комментарии, реакции
...
-- RLS: публичное чтение опубликованного, запись — только сервис-ключом
...
-- стартовые источники
-- Русскоязычные (проверены — RSS отдаётся). Фильтр-ИИ отсеет неподходящее по тону.
insert into sources (name, rss_url, default_category) values
  ...
  ('N+1',                'https://nplus1.ru/rss', 1),
  -- Зарубежные (ИИ-редактор переводит на русский)
...
-- Права для ролей (иначе воркер с service-ключом получает permission denied).
